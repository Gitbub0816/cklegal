// clearkey-legal.jsx
// Deploy as a separate Cloudflare Pages project at legal.clearkeysolutions.com
// npm install react-router-dom

import { useState } from "react";
import { BrowserRouter, Routes, Route, Link, useNavigate, useParams } from "react-router-dom";

const CO  = "ClearKey Solutions LLC";
const PROD = "ClearKey Connect";
const EFF  = "June 19, 2026";
const ADR  = "Silicon Valley, California";
const LEG  = "legal@clearkey.solutions";
const PRIV = "https://legal.clearkey.solutions/privacy-policy";
const SMS  = "https://legal.clearkey.solutions/policy/sms-policy/policy";
const YR   = "2026";
...
// Organise into categories for hub display
const CATEGORIES = [
  { label:"Agreements",        color:"#8B6914" },
  { label:"Privacy & Data",    color:"#1D4ED8" },
  { label:"Billing & Payments",color:"#065F46" },
  { label:"Security & Compliance",color:"#7C3AED" },
  { label:"AI & Technology",   color:"#B45309" },
  { label:"Technical",         color:"#0F766E" },
  { label:"Legal",             color:"#9F1239" },
  { label:"Operations",        color:"#374151" },
  { label:"Communications",    color:"#1E40AF" },
  { label:"Disclaimers",       color:"#92400E" },
];

// SMS Policy content
const SMS_POLICY_SECTIONS = [
  {h:"1. Introduction",c:[
...
function Logo({ height = 40 }) {
  ...
  if (fail) return(
    <div className="header-logo-fb">
      <div>
        <div className="wt">CLEARKEY</div>
        <div className="wb">SOLUTIONS</div>
      </div>
    </div>
  );
  return <img src="/logo.png" alt="ClearKey Solutions" style={{height,width:"auto"}} onError={()=>setFail(true)}/>;
}
...
function Header({ showBack }) {
  ...
  return(
    <header className="site-header">
      <a className="header-logo" href="/" onClick={e=>{e.preventDefault();nav("/");}}>
        <Logo height={38}/>
      </a>
      <span className="header-badge">Legal & Policy Center</span>
      {showBack && (
...
function Footer() {
  ...
  return(
    <footer className="site-footer">
      <span className="site-footer-copy">© {YR} {CO}. All rights reserved.</span>
      <div className="site-footer-links">
        <a href="/" onClick={e=>{e.preventDefault();nav("/")}}>Policy Hub</a>
        <a href="/privacy-policy" onClick={e=>{e.preventDefault();nav("/privacy-policy")}}>Privacy</a>
        <a href="/terms-of-service" onClick={e=>{e.preventDefault();nav("/terms-of-service")}}>Terms</a>
  ...
  return(
    <>
      ...
      <div className="hub-wrap">
        <div className="hub-eyebrow">Legal & Policy Center</div>
        <h1 className="hub-title">Agreements, Disclosures &amp; Policies</h1>
        <p className="hub-sub">
          Static public endpoints for {CO} agreements, disclosures, operational
          policies, and customer rights. All documents are effective {EFF}.
          Questions? Contact <a href={`mailto:${LEG}`} style={{color:"var(--gold-d)"}}>{LEG}</a>.
        </p>
        <div className="hub-divider"/>

        {CATEGORIES.map(cat => {
          const items = POLICIES.filter(p => p.category === cat.label);
          if (!items.length) return null;
          return(
            <div key={cat.label} style={{marginBottom:"40px"}}>
              <div className="hub-section-label" style={{color:cat.color}}>{cat.label}</div>
              <div className="hub-grid">
                {items.map(p=>(
                  <a key={p.slug} className="policy-card"
                    href={`/${p.slug}`}
                    onClick={e=>{e.preventDefault();nav(`/${p.slug}`);}}>
                    <div className="policy-card-title">{p.title}</div>
                    <div className="policy-card-desc">{p.description}</div>
                    <span className="policy-card-arrow">View document →</span>
                  </a>
                ))}
              </div>
            </div>
        ...
        <div style={{marginBottom:"40px"}}>
          <div className="hub-section-label" style={{color:"#0F766E"}}>SMS Communications</div>
          <div className="hub-grid">
            {[
              {href:"/policy/sms-policy/opt-in",title:"SMS Opt-In",desc:"Opt in to receive text message communications from ClearKey Solutions."},
              {href:"/policy/sms-policy/unsubscribe",title:"SMS Unsubscribe",desc:"Opt out of SMS communications and confirm your unsubscribe request."},
              {href:"/policy/sms-policy/policy",title:"SMS Communication Policy",desc:"Full policy governing how we send, what we send, and your rights regarding SMS messages."},
            ].map(s=>(
              <a key={s.href} className="policy-card" href={s.href}
                onClick={e=>{e.preventDefault();nav(s.href);}}>
                <div className="policy-card-title">{s.title}</div>
                <div className="policy-card-desc">{s.desc}</div>
                <span className="policy-card-arrow">View →</span>
              </a>
            ))}
          </div>
        </div>
...
// ─── Table renderer ───────────────────────────────────────────────
function PolicyTable({ cols, rows }) {
  return(
...
function PolicyPage() {
  ...
  if (!policy) return(
    <>
      <Header showBack/>
      <div className="doc-wrap" style={{textAlign:"center",paddingTop:"100px"}}>
        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"48px",fontWeight:300,color:"#C9A033",marginBottom:"16px"}}>404</div>
        <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"28px",fontWeight:300,marginBottom:"16px"}}>Document Not Found</h1>
        <p style={{color:"var(--txt-m)",marginBottom:"32px"}}>The policy document you requested does not exist.</p>
        <button className="doc-footer-back" onClick={()=>nav("/")}>← Return to Policy Hub</button>
      </div>
      <Footer/>
    </>
  );
    ...
    <>
      ...
      <div className="doc-wrap">
        ...
        <div style={{marginBottom:"8px"}}>
          <span style={{fontFamily:"'Raleway',sans-serif",fontSize:"9px",fontWeight:700,letterSpacing:"3px",textTransform:"uppercase",color:"var(--txt-m)"}}>{policy.category}</span>
        </div>
        <h1 className="doc-title">{policy.title}</h1>

        <div className="doc-meta">
          <span className="doc-meta-item">Effective Date: {EFF}</span>
          <span className="doc-meta-item">Last Revised: {EFF}</span>
          <span className="doc-meta-item">{CO}</span>
        </div>
        ...
        {policy.sections.map((section, i) => (
          <div key={i} id={`s${i}`} className="doc-section">
            <h2 className="doc-section-heading">{section.h}</h2>
            {section.c.map((para,j) => (
            ...
            {section.table && <PolicyTable cols={section.table.cols} rows={section.table.rows}/>}
          </div>
        ))}

        {/* Footer */}
        <div className="doc-footer">
          <p className="doc-footer-contact">
            Questions about this {policy.title}? Contact us at{" "}
            <a href={`mailto:${LEG}`}>{LEG}</a> or write to {CO}, {ADR}.
          </p>
          <button className="doc-footer-back" onClick={()=>nav("/")}>← Back to Policy Hub</button>
        </div>
      </div>
      <Footer/>
    </>
  ...
  return(
    <div className="sms-bg">
      <div className="sms-card">
        <div className="sms-logo"><Logo height={44}/></div>

        {done ? (
          <div className="sms-success">
            <div className="sms-success-icon">✓</div>
            <div className="sms-success-title">You're opted in</div>
            <p className="sms-success-body">
              You will receive text messages from {CO} at the number you provided.
              Reply STOP at any time to unsubscribe. Message and data rates may apply.
            </p>
          </div>
        ) : (
          <>
            <h1 className="sms-title">Opt In to SMS Messages</h1>
            <p className="sms-subtitle">
              {CO} would like to send you text messages including service alerts,
              account notifications, and optional promotional updates.
            </p>

            <div className="sms-scope">
              <p><strong>What you'll receive:</strong></p>
              <p>
                Account security alerts · Service disruption notices · Subscription
                reminders · Optional product updates and promotions (if selected below)
              </p>
            </div>

            <label className="sms-input-label" htmlFor="sms-phone">Mobile Phone Number</label>
            <input
              id="sms-phone"
              className="sms-input"
              type="tel"
            ...
            <div className="sms-disclaimer">
              By clicking "Opt In &amp; Agree," you expressly consent to receive recurring
              automated text messages from {CO} at the mobile number provided. Consent is not
              a condition of purchase. Message frequency varies. Message and data rates may apply.
              Reply <strong>HELP</strong> for help, <strong>STOP</strong> to cancel. View our{" "}
              <a href="/policy/sms-policy/policy" onClick={e=>{e.preventDefault();nav("/policy/sms-policy/policy");}}>
                SMS Communication Policy
              </a>{" "}and{" "}
              <a href="/privacy-policy" onClick={e=>{e.preventDefault();nav("/privacy-policy");}}>
                Privacy Policy
              </a>.
            </div>

            <button className="sms-btn" onClick={handleOptIn}>Opt In &amp; Agree</button>
            <button className="sms-btn-secondary" onClick={()=>nav("/")}>No thanks</button>

            <div className="sms-links">
              <a href="/policy/sms-policy/policy" onClick={e=>{e.preventDefault();nav("/policy/sms-policy/policy");}}>SMS Policy</a>
              <a href="/privacy-policy" onClick={e=>{e.preventDefault();nav("/privacy-policy");}}>Privacy Policy</a>
              <a href="/policy/sms-policy/unsubscribe" onClick={e=>{e.preventDefault();nav("/policy/sms-policy/unsubscribe");}}>Unsubscribe</a>
            </div>
          </>
        )}
      </div>
...
function SMSUnsubscribe() {
  ...
  const handleUnsub = () => {
    if (phone.replace(/\D/g,"").length < 10) { alert("Please enter the phone number you opted in with."); return; }
    setDone(true);
  };
    ...
    <div className="sms-bg">
      <div className="sms-card">
        ...
        {done ? (
          <div className="sms-success">
            <div className="sms-success-icon">✓</div>
            <div className="sms-success-title">You've been unsubscribed</div>
            <p className="sms-success-body">
              We've removed your number from our SMS list. You may still receive
              transactional security messages necessary for your account.
              If you continue to receive unwanted messages, contact{" "}
              <a href={`mailto:${LEG}`} style={{color:"var(--gold-d)"}}>{LEG}</a>.
            </p>
          </div>
        ) : (
          <>
            <h1 className="sms-title">Unsubscribe from SMS</h1>
            <p className="sms-subtitle">
              Enter the phone number you used to opt in. We'll remove it from our
              SMS marketing list within 10 business days.
            </p>

            <div className="sms-scope">
              <p>
                You may still receive <strong>transactional</strong> messages such as
                security alerts, one-time passcodes, and critical account notifications
                as these are required to maintain account security.
              </p>
            </div>

            <label className="sms-input-label" htmlFor="unsub-phone">Mobile Phone Number to Unsubscribe</label>
            <input
              id="unsub-phone"
              className="sms-input"
              type="tel"
            ...
            <button className="sms-btn-danger" onClick={handleUnsub}>Confirm Unsubscribe</button>
            <button className="sms-btn-secondary" onClick={()=>nav("/")}>Cancel</button>

            <div className="sms-links">
              <a href="/policy/sms-policy/policy" onClick={e=>{e.preventDefault();nav("/policy/sms-policy/policy");}}>SMS Policy</a>
              <a href="/policy/sms-policy/opt-in" onClick={e=>{e.preventDefault();nav("/policy/sms-policy/opt-in");}}>Opt In</a>
            </div>
          </>
        )}
      </div>
...
function SMSPolicyPage() {
  ...
  return(
    <>
      ...
      <div className="doc-wrap">
        <div style={{marginBottom:"8px"}}>
          <span style={{fontFamily:"'Raleway',sans-serif",fontSize:"9px",fontWeight:700,letterSpacing:"3px",textTransform:"uppercase",color:"var(--txt-m)"}}>SMS Communications</span>
        </div>
        <h1 className="doc-title">SMS Communication Policy</h1>
        <div className="doc-meta">
          <span className="doc-meta-item">Effective Date: {EFF}</span>
          <span className="doc-meta-item">Last Revised: {EFF}</span>
          <span className="doc-meta-item">{CO}</span>
        ...
        <div className="doc-toc">
          <div className="doc-toc-title">Contents</div>
          <ol>
            {SMS_POLICY_SECTIONS.map((s,i)=>(
              <li key={i}><a href={`#sms${i}`}>{s.h}</a></li>
            ))}
          </ol>
        </div>
        {SMS_POLICY_SECTIONS.map((section,i)=>(
          <div key={i} id={`sms${i}`} className="doc-section">
            <h2 className="doc-section-heading">{section.h}</h2>
            {section.c.map((para,j)=>(
              <p key={j} dangerouslySetInnerHTML={{
        ...
        {/* Related links */}
        <div style={{marginTop:"40px",padding:"24px",background:"var(--bg2)",border:"1px solid var(--border)"}}>
          <div style={{fontFamily:"'Raleway',sans-serif",fontSize:"9px",fontWeight:700,letterSpacing:"3px",textTransform:"uppercase",color:"var(--txt-m)",marginBottom:"16px"}}>Related SMS Pages</div>
          <div style={{display:"flex",gap:"16px",flexWrap:"wrap"}}>
            <button onClick={()=>nav("/policy/sms-policy/opt-in")} style={{background:"var(--gold)",color:"#FAFAF8",padding:"10px 20px",border:"none",fontFamily:"'Raleway',sans-serif",fontWeight:700,fontSize:"10px",letterSpacing:"2px",textTransform:"uppercase",cursor:"pointer"}}>Opt In to SMS</button>
            <button onClick={()=>nav("/policy/sms-policy/unsubscribe")} style={{background:"transparent",color:"var(--txt-s)",padding:"9px 19px",border:"1px solid rgba(0,0,0,.15)",fontFamily:"'Raleway',sans-serif",fontWeight:600,fontSize:"10px",letterSpacing:"2px",textTransform:"uppercase",cursor:"pointer"}}>Unsubscribe</button>
          </div>
        </div>
  ...
  return(
    <BrowserRouter>
      <Routes>
        ...
        <Route path="/policy/sms-policy/opt-in"     element={<SMSOptIn/>}/>
        <Route path="/policy/sms-policy/unsubscribe" element={<SMSUnsubscribe/>}/>
        <Route path="/policy/sms-policy/policy"     element={<SMSPolicyPage/>}/>
